...
1) Compile our modified Caffe:
    ...
    - In case you want to modify the config file, do it on the `then copy the filename as `Makefile.config` by running `cp Makefile.config.example Makefile.config`
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTU1NzQ0NzQyMSwtNDc1NDY4NTMxLDIwMz
