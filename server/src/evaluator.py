...
def draw_line(frame_index, from_point, to_point):
    ...
    frame_path = output_manager.get_output_dirs_dict()['frames_path'] + '/annotated_frame_{}.jpg'.format(frame_index)
    frame = cv2.imread(frame_path)
    annotated_frame = cv2.line(frame, to_point, from_point, (255, 0, 0))
    cv2.imwrite(frame_path, annotated_frame)


errors = [{'check_hand_crossed_the_middle_line': check_hand_crossed_the_middle_line}]
