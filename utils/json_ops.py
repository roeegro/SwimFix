...
def filter_json(json_path, filters, out_path=None):
    ...
    parsed['annotations'] = filtered_annots
...
if __name__ == '__main__':
    ...
    openpose_train_raziel_dataset_path = openpose_train_path + "dataset/COCO/cocoapi/images/raziel"
    if not os.path.exists(openpose_train_raziel_dataset_path):
        os.mkdir(openpose_train_raziel_dataset_path)

    annotator_dataset_path = openpose_train_path + "datasets/Segmentation Test"

    filtered_json_path = openpose_train_annots_path + "person_keypoints_raziel.json"
    filter_json(openpose_train_annots_path + "Segmentation Test.json",
                ['height', 'width', 'id', 'path', 'image_id', 'segmentation', 'bbox', 'keypoints',
                 "annotated", "file_name", "num_keypoints", "area", "iscrowd"],
                filtered_json_path)
